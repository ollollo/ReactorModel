...
def main():
    ...
    # Основное содержимое
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Анализ модели", "🎯 Оптимизация", "🔬 Кинетическая модель", "📈 3D поверхность"])
    ...
    with tab3:
        st.header("Кинетическая модель реактора")
        
        if st.button("Расчитать кинетичсекую модель"):
            with st.spinner("Расчет PFR модели..."):
                ...
                fig.update_layout(
                    title='Изменение концентраций во времени (кинетическая модель)',
                    xaxis_title='Время, с',
